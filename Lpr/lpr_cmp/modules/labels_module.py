...
class LabelGenerator():
    ...
    def add_legend(self, image_labeled):
        ...
        image_labeled_legend = np.empty(
            (height_image+height_image_legend, width_image, 3), dtype=np.uint8)
    ...
    # def make_text_label_transparent(self, characters):
        # cv2.addWeighted
    ...
    def make_text_label(self,
        ...
        if _class >= self.number_color:
            print("class number %d out of total classes 0~%d" %
        ...
        subimages = [self.templates[size][c if c in self.letters else " "]
                     for c in characters]
        image_grayscale = np.hstack(subimages)

        if self.bright_bg[_class]:
            image_bg = np.expand_dims(
                image_grayscale, axis=-1)/51 * (np.uint8(self.pixels_bg[_class].reshape(1, 3))/5)
        else:
            image_bg = 255 - np.expand_dims(image_grayscale, axis=-1)/51 * (
                51-np.uint8(self.pixels_bg[_class].reshape(1, 3))/5)

        return image_bg

    def draw_bbox_with_text(self, image_labeled, text, size, _class, bbox_coord): 
        ...
        # exp : bbox = 391 183 533 236, relative to whole image
        ...
        actual_width = left_bbox + width_subimage
        if actual_width > width_image:
            left_subimage = left_bbox - (actual_width - width_image)
        else:
        ...
        # ** for label location
        # print('subimage :', left_subimage, top_subimage, right_subimage, bottom_subimage)
        # exp : subimage = 391 111 728 186, relative to whole image

        # ** for yolo rectangle drawing
        ...
        # ** modify label location, label is added (subimage_text[] is label)
        # width_subimage = right_subimage - left_subimage
        if top_bbox <= 82:
            shift_height = bottom_subimage - top_subimage
            image_labeled[bottom_bbox:bottom_bbox+shift_height, left_subimage: right_subimage, :] = \
    ...
    def _get_pixel_value(self):
        ...
        for i in range(self.number_color):
            pixel_hsv[0,0,0] = (i*123457%self.number_color)*180/self.number_color
